    ...
    <>
      ...
      <div className="pt-24 min-h-screen bg-slate-50">
        ...
        <div className="container-custom py-12 print-page">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Image */}
            <div 
              className="bg-white rounded-3xl shadow-card overflow-hidden aspect-square flex items-center justify-center cursor-pointer group relative"
              onClick={() => { if (product.imageUrl) setLightboxOpen(true); }}
            >
              {product.imageUrl ? (
                <>
                  <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center">
                    <div className="bg-white/90 text-slate-800 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity transform scale-90 group-hover:scale-100 shadow-sm">
      ...
      {product.imageUrl && (
        <Lightbox
          ...
          slides={[{ src: product.imageUrl, alt: product.name }]}
          plugins={[Zoom]}
